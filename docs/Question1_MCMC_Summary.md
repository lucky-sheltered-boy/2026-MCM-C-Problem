...
| 采样方法 | 使用次数 | 说明 |
|----------|----------|------|
| mcmc_smooth | 277 | MCMC平滑采样（普通周） |
| mcmc_prior | 14 | MCMC先验采样（第一周） |
| mcmc_finale | 34 | MCMC决赛周采样 |
| prior_only (no elim) | 20 | 无淘汰周使用先验 |
| smooth_projection (no elim) | 23 | 无淘汰周平滑投影 |
...
| 指标 | 平均值 | 说明 |
|------|--------|------|
| 接受率 | ~0.70 | 在最优范围[0.2, 0.5]附近 |
| 有效样本量 | ~9500 | thinning=10后保留 |
...
某些周无人淘汰（如伤病退出替换），使用平滑投影保持与前一周相似的投票分布。

### 5.3 决赛周

决赛周使用**排名约束**替代淘汰约束：
- 约束条件：第1名综合分 ≥ 第2名 ≥ 第3名 ≥ ...
- 使用非严格不等式，因为某些决赛（如S28）在严格不等式下无解
- 34个决赛周全部成功采样

### 5.4 极端案例

**Season 28, Week 6 - Sailor Brinkley-Cook**
- 评委得分排名：第1名
- 实际结果：被淘汰
- 这意味着粉丝投票极低
...
| method | 采样方法 (mcmc_smooth/mcmc_finale/等) |
...
├── results/
│   └── mcmc_smooth_results.csv   # 最终估计结果（368周）
...
1. **100%一致性**：所有368周的估计结果都满足历史淘汰/排名约束
2. **决赛周处理**：34个决赛周全部成功采样，使用非严格排名约束
3. **平滑性**：通过顺序生成策略，平均周间变化仅0.1166
4. **鲁棒性**：能够处理各种特殊案例（极端淘汰、多人淘汰、无淘汰周等）
5. **可解释性**：每位选手每周都有明确的粉丝投票份额估计
